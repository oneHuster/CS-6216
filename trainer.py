...
class Trainer(object):
    ...
    def __init__(self, arg):
        ...
        if self.neighbor in ['gat', 'gcn']:
            self.matcher = GraphMatcher(self.embed_dim, self.num_symbols, use_pretrain=self.use_pretrain, embed=self.symbol2vec, dropout=self.dropout, batch_size=self.batch_size, process_steps=self.process_steps, finetune=self.fine_tune, aggregate=self.aggregate)
        else:
