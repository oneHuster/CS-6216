...
class GraphMatcher(nn.Module):
    ...
    def __init__(self, embed_dim, num_symbols, use_pretrain=True, embed=None, dropout=0.2, batch_size=64, process_steps=4, finetune=False, aggregate='max'):
        ...
        if aggregate == 'gcn':
            self.support_encoder = GCNEncoder(d_model, d_model)
            self.query_encoder = GCNEncoder(d_model, d_model)
        elif aggregate == 'gat':
            self.support_encoder = GATEncoder(d_model, d_model)
            self.query_encoder = GATEncoder(d_model, d_model)
        # else:
    ...
    def neighbor_encoder(self, connections, num_neighbors):
        ...
        rel_embeds = self.dropout(self.symbol_emb(relations)) # (batch, 200, embed_dim) NELL (128, 50, 100)
    ...
    def forward(self, query, support, query_meta=None, support_meta=None):
        ...
        '''
        # import pdb; pdb.set_trace()
        query_left_connections, query_left_degrees, query_right_connections, query_right_degrees = query_meta # [torch.Size([128, 50, 2]), torch.Size([128]), torch.Size([128, 50, 2]), torch.Size([128])]
        support_left_connections, support_left_degrees, support_right_connections, support_right_degrees = support_meta # [torch.Size([1, 50, 2]), torch.Size([1]), torch.Size([1, 50, 2]), torch.Size([1])]
...
class EmbedMatcher(nn.Module):
    ...
    def neighbor_encoder(self, connections, num_neighbors):
        ...
        relations = connections[:,:,0].squeeze(-1)
        entities = connections[:,:,1].squeeze(-1)
        rel_embeds = self.dropout(self.symbol_emb(relations)) # (batch, 200, embed_dim) NELL (batch, 50, 100)
        ent_embeds = self.dropout(self.symbol_emb(entities)) # (batch, 200, embed_dim)
